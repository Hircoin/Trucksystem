                ...
                <tbody>
                    {rows.map(row => (
                        <tr key={row.id} className="data-grid-row">
                            <td>{`${row.trucknumber}( ${row.tonesLoad})-${row.driverename}`}</td>
                            <td className="home-subscribe">
                                <Link to={`${row.revenue}`}><EditButton>View</EditButton></Link>
                            </td>
                        </tr>
                    ))}
                </tbody>
...
const getRandomDirection = () => {
    ...
    // const rows = Object.values(property).map(item => ({
    //     company: `${item.propertyName}`,
    //     revenue: `/add/Paybills/flore?property=${item._id}`
    // }));
    const rows = shipments.map(item => ({
        id: item._id,
        trucknumber: item.trucknumber,
        tonesLoad: item.tonesLoad,
        driverename: item.driverename,
        revenue: `/shipment/details/${item._id}`
    }));
