...
//     //const market = searchParams.get('property');
...
//                 let response;
//                 if (renter) {
//                     response = await API.getAllRentbills({ rentId: renter});
//                 }else {
//                     response = await API.getAllRentbills({ rentId: renter});
//                 }
...
//     return date.toLocaleDateString('en-GB', options); // Formats to "14 November 2024"
// }
...
//     // Sample data
//     // const rows = [
//     //     { id: 1, company: 'Rentbill', revenue:`/Rentbill/${id}` },
//     //     { id: 2, company: 'Paybills', revenue: `/Paybills/${id}` },
//     //     { id: 3, company: 'Report', revenue:`/Report/${id}`},
//     //     { id: 4, company: 'Details', revenue: `/AllRenterDetail/Details/${id}` },
...
//     const rows = Object.values(rentbill).map(item => ({
//         company: `${formatDate(item.rentStartingDate)} to ${formatDate(item.rentEndingDate)} (${item.totalAmount})`,
//         revenue: `/add/Paybills?rentbillId=${item._id}`
//     }));
//     // Define columns
//     const columns = [
...
// export default AddpaybillRentbill ;
...
function DataGridPremium({ rows, columns }) {
    ...
    // Extract unique companies where the condition is met
    const uniqueCompanies = [
        ...new Set(
            rows
                .filter(row => row.totalAmount !== 0 && row.billStatus !== "paid")
                .map(row => row.company)
        )
    ];
                        ...
                        <React.Fragment key={company}>
                            <tr
                                ...
                                onClick={() =>
                                    onRowClick(
                                        company,
                                        rows.find(row => row.company === company).revenue
                                    )
                                }
                            >
                                <td>{company}</td>
                                ...
                                rows
                                    ...
                                    .map(row => (
                                        <tr key={row.id} className="data-grid-details-row">
                                            <>
                                                <td>{`Details for ${row.company}`}</td>
                                                <td className="home-subscribe">
                                                    <Link to={`${portfoliosize}`}>
